...
class MainActivity : AppCompatActivity() {
    ...
    private data class InputEvent(val airHeight: Int = 6, val testButton: Boolean = false, val serviceButton: Boolean = false)
    ...
    private var mUseFrontCamera = true
    private var mZoneStates = BooleanArray(6) { false }
    private var mCameraAirSensitivity = 0.15f
    private var mLastFrame: ByteArray? = null
    private lateinit var mZoneOverlay: ZoneOverlayView
    private val CAMERA_PERMISSION_REQUEST = 1001
    private var mRoiCenterX = 0.5f
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mUseFrontCamera = app.useFrontCamera.value()
        mCameraAirSensitivity = app.cameraAirSensitivity.value()
        ...
        findViewById<Button>(R.id.button_flip_camera).setOnClickListener {
            ...
            app.useFrontCamera.update(mUseFrontCamera)
            stopCamera()
            startCamera()
        }
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            ...
            val selector = if (mUseFrontCamera) CameraSelector.DEFAULT_FRONT_CAMERA else CameraSelector.DEFAULT_BACK_CAMERA
            ...
            imageAnalysis.setAnalyzer(ContextCompat.getMainExecutor(this)) { imageProxy ->
                processFrameForAir(imageProxy)
            }

            try {
                mCameraProvider?.unbindAll()
                mCameraProvider?.bindToLifecycle(this, selector, preview, imageAnalysis)
            } catch (e: Exception) {
    ...
    private fun processFrameForAir(imageProxy: ImageProxy) {
        ...
        mCurrentAirHeight = 6
        for (zone in 0 until numOfAirBlock) {
            if (newZoneStates[zone]) {
                mCurrentAirHeight = zone
                break
            }
        }
        ...
        if (mDebugInfo) {
            val textInfo = findViewById<TextView>(R.id.text_info)
            runOnUiThread {
                textInfo.text = getString(R.string.debug_info_camera, mCurrentAirHeight, newZoneStates.toList().toString())
            }
        }
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        ...
        if (requestCode == CAMERA_PERMISSION_REQUEST && grantResults.firstOrNull() == PackageManager.PERMISSION_GRANTED) {
            ...
            Toast.makeText(this, "Camera permission required for air detection", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    override fun onResume() {
        ...
        mCameraAirSensitivity = app.cameraAirSensitivity.value()
        mRoiCenterX = app.roiCenterX.value() / 100f
            ...
            doInBackground = {
                ...
                if (mTCPMode) {
                    ...
                    while (!mExitFlag) {
                        ...
                        val event = InputEvent(mCurrentAirHeight, mTestButton, mServiceButton)
    ...
    private fun applyKeys(event: InputEvent, buffer: IoBuffer): IoBuffer {
        return buffer.apply {
            ...
            // slider stays all zeros — no touch input
            if (mEnableAir) {
                val currentTime = System.currentTimeMillis()
                if (currentTime - mLastAirUpdateTime > airUpdateInterval) {
                    mLastAirHeight += if (mLastAirHeight < mCurrentAirHeight) 1
                    else if (mLastAirHeight > mCurrentAirHeight) -1
                    else 0
